...
ALTER TABLE DeliveryInfo ADD COLUMN IF NOT EXISTS scheduledStartTime    TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP;

-- Will be non-null if the deliverer has started the delivery (begin route to Donor for pickup).
ALTER TABLE DeliveryInfo ADD COLUMN IF NOT EXISTS startTime             TIMESTAMPTZ DEFAULT NULL;

-- Will be non-null if the Food Listing has been picked up from the Donor and is on-route to the Receiver.
ALTER TABLE DeliveryInfo ADD COLUMN IF NOT EXISTS pickUpTime            TIMESTAMPTZ DEFAULT NULL;

-- Will be non-null if the Food Listing has been dropped off at the Receiver destination.
ALTER TABLE DeliveryInfo ADD COLUMN IF NOT EXISTS dropOffTime           TIMESTAMPTZ DEFAULT NULL;
