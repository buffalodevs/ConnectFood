 ...
 */
export async function getDeliveries(filters: FoodListingFilters, myAppUserKey: number, myGPSCoordinate: GPSCoordinate): Promise <FoodListing[]> {
   
    sanitizeFilters(filters);

    try {
        const queryResult: QueryResult = await queryGetFoodListings(filters, myAppUserKey);
        return await generateResultArray(queryResult, myGPSCoordinate);
    }
    catch (err) {
 ...
 */
async function generateResultArray(queryResult: QueryResult, myGPSCoordinate: GPSCoordinate): Promise <FoodListing[]> {
